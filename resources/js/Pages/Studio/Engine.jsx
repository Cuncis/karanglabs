...
function Field({ field, value, onChange }) {
    const base = 'w-full rounded-lg border border-[#222] bg-[#0D0D0D] px-3 py-2 text-sm text-[#EDEDED] placeholder-[#555] focus:border-emerald-400/50 focus:outline-none focus:ring-1 focus:ring-emerald-400/30';

    return (
        <label className="block">
            <span className="mb-1.5 flex items-center gap-1.5 text-sm font-medium text-[#D4D4D8]">
                {field.label}
            ...
            {field.hint && <span className="mt-1 block text-xs text-[#666]">{field.hint}</span>}
        </label>
    );
}
        ...
        <StudioLayout>
            ...
            <div className="mb-8 flex items-center gap-4">
                ...
                <div>
                    <div className="flex items-center gap-2">
                        <h1 className="text-2xl font-bold tracking-tight text-white">{engine.name}</h1>
                        <span className="font-mono text-xs text-[#555]">{engine.code}</span>
                        {engine.star && <span className="text-amber-400">★</span>}
                    </div>
                    <p className="text-sm text-[#888]">{engine.tagline}</p>
                </div>
            </div>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                ...
                <div className="space-y-4 rounded-xl border border-[#222] bg-[#111] p-6">
                    <h2 className="text-sm font-semibold uppercase tracking-widest text-[#666]">Brief</h2>
                ...
                <div className="lg:sticky lg:top-6 lg:self-start">
                    <div className="rounded-xl border border-[#222] bg-[#0D0D0D]">
                        <div className="flex items-center justify-between border-b border-[#222] px-5 py-3">
                            <span className="flex items-center gap-2 text-sm font-medium text-white">
                                <Wand2 className="h-4 w-4 text-emerald-400" /> Prompt siap-pakai
                            </span>
                            <button
                                type="button"
                                onClick={copyPrompt}
                        ...
                        <pre className="max-h-[52vh] overflow-auto whitespace-pre-wrap p-5 font-mono text-[12px] leading-relaxed text-[#C8C8C8]">
                            {prompt}
                        </pre>
                        <div className="flex items-center justify-between gap-3 border-t border-[#222] px-5 py-3">
                            <p className="text-xs text-[#666]">Paste ke ChatGPT · Claude · Gemini · v0 · Lovable</p>
                            <button
                                type="button"
                                onClick={save}
                                disabled={!hasInput || form.processing}
                                className="inline-flex items-center gap-1.5 rounded-md border border-[#333] px-3 py-1.5 text-xs font-medium text-[#EDEDED] transition-colors hover:border-[#555] hover:bg-[#161616] disabled:cursor-not-allowed disabled:opacity-40"
                            >
                    ...
                    {recentProjects.length > 0 && (
                        <div className="mt-6">
                            <h2 className="mb-3 text-sm font-semibold uppercase tracking-widest text-[#666]">Tersimpan</h2>
                            <div className="space-y-2">
                                {recentProjects.map((p) => (
                                    <div key={p.id} className="flex items-center justify-between gap-3 rounded-lg border border-[#222] bg-[#111] px-4 py-3">
                                        <div className="min-w-0">
                                            <div className="truncate text-sm font-medium text-white">{p.title}</div>
                                            <div className="text-xs text-[#666]">{timeAgo(p.created_at)}</div>
                                        </div>
                                        <div className="flex items-center gap-1">
                                            <button
                                                type="button"
                                                onClick={() => { navigator.clipboard?.writeText(p.prompt); }}
                                                className="rounded-md p-2 text-[#888] transition-colors hover:bg-[#1A1A1A] hover:text-white"
                                                title="Copy prompt"
                                            >
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => remove(p.id)}
                                                className="rounded-md p-2 text-[#888] transition-colors hover:bg-red-500/10 hover:text-red-400"
                                                title="Hapus"
                                            >
