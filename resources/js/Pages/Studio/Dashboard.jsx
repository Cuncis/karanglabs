    ...
    return (
        <StudioLayout>
            ...
            <div className="mb-10">
                ...
                <h1 className="mt-4 text-3xl font-bold tracking-tight text-white">Halo, {firstName} 👋</h1>
            ...
            {/* Engines */}
            <h2 className="mb-4 text-sm font-semibold uppercase tracking-widest text-[#666]">Buat website baru</h2>
                ...
                {ENGINES.map((engine) => {
                    ...
                    return (
                        <Link
                            ...
                            className="group flex flex-col rounded-xl border border-[#222] bg-[#111] p-5 transition-all hover:border-[#3a3a3a] hover:bg-[#161616]"
                        >
                            ...
                            <h3 className="mt-4 flex items-center gap-1.5 text-base font-semibold text-white">
                                {engine.name}
                                {engine.star && <span className="text-amber-400">★</span>}
                            </h3>
                            <p className="mt-1.5 flex-1 text-sm leading-relaxed text-[#888]">{engine.tagline}</p>
                            <span className="mt-4 inline-flex items-center gap-1 text-sm font-medium text-emerald-400 opacity-0 transition-opacity group-hover:opacity-100">
            ...
            {recentProjects.length > 0 && (
                <div className="mt-12">
                    <h2 className="mb-4 text-sm font-semibold uppercase tracking-widest text-[#666]">Project terakhir</h2>
                    <div className="divide-y divide-[#1a1a1a] overflow-hidden rounded-xl border border-[#222] bg-[#111]">
                        {recentProjects.map((p) => (
                            <Link
                                key={p.id}
                                href={route('studio.engine', { engine: p.engine })}
                                className="flex items-center justify-between gap-4 px-5 py-4 transition-colors hover:bg-[#161616]"
                            >
                                <div className="min-w-0">
                                    <div className="truncate font-medium text-white">{p.title}</div>
                                    <div className="text-xs text-[#666]">{engineName(p.engine)} · {timeAgo(p.created_at)}</div>
                                </div>
                                <ArrowRight className="h-4 w-4 flex-shrink-0 text-[#555]" />
                            </Link>
                        ))}
                    </div>
                </div>
